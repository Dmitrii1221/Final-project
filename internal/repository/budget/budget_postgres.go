...
func (r *PostgresRepo) Insert(ctx context.Context, name string) (domain.Budget, error) {
	query, args, err := psql.
		Insert("Budgets").
		Columns("name").
		Values(name).
	...
	var b domain.Budget
	err = r.pool.QueryRow(ctx, query, args...).Scan(&b.ID, &b.Name, &b.CreatedAt, &b.UpdatedAt)
	if err != nil {
